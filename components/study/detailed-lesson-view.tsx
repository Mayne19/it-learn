          ...
          <>
            ...
            <div className="space-y-6">
              {lesson.sections.map((section, i) => (
                <div key={i} className="space-y-2 border-b border-border/50 pb-5 last:border-0 last:pb-0">
                  <h3 className="scroll-m-20 text-xl font-semibold tracking-tight sm:text-2xl">{section.heading_de}</h3>
                  <p className="leading-7">{showFr ? section.explanation_fr : section.explanation_de}</p>
                  ...
                  <div className="flex items-start gap-2 rounded-lg bg-ring/10 border border-ring/20 p-3">
                    <Lightbulb className="h-4 w-4 flex-shrink-0 text-ring mt-0.5" />
                    <p className="text-sm">{section.memory_tip_fr}</p>
                  </div>
                </div>
              ))}
            </div>
