...
export async function POST(req: Request) {
  ...
  // Bug Hunt n'a de sens que pour du code — le composant appelant ne
  // devrait déjà l'afficher que si has_code, mais on protège aussi côté
  // serveur (voir ingest/route.ts pour le même réflexe sur d'autres routes).
  if (!chapter.has_code) {
    return Response.json(
  ...
  const prompt = buildBugHuntPrompt(chapter)
  ...
  try {
    ...
    console.error('[study/bug-hunt]', message)
