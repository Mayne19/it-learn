...
export async function POST(req: Request) {
  ...
  const prompt = buildFillBlankPrompt(chapter)

  try {
    const text = await callClaude({ model: 'claude-haiku-4-5', prompt, maxTokens: 1500 })
    const parsed = extractJSON(text)
    return Response.json(parsed)
  } catch (e) {
    const message = e instanceof Error ? e.message : String(e)
    console.error('[study/fill-blank]', message)
