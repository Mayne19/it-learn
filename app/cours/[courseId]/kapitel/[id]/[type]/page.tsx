...
export default function ExercisePage() {
  ...
  }, [courseId, chapterId, exerciseType, fillBlankMode])
  ...
  const handleResult = async (correct: boolean) => {
    ...
    try {
      if (isSupabaseConfigured() && user) {
        await updateProgress(user.id, courseId, chapterId, exerciseType, correct)
      } else {
        localUpdateProgress(courseId, chapterId, exerciseType, correct)
      }
    } catch (error) {
  ...
  const handleBack = () => router.push(`/cours/${courseId}/kapitel/${chapterId}`)
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <div className="flex items-start gap-4">
        <Link href={`/cours/${courseId}/kapitel/${chapterId}`} className="inline-flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-md transition-colors hover:bg-accent">
      ...
      {!loading && !error && exercise && (
        <>
          {exerciseType === 'mcq' && (
            ...
            <MCQExercise data={exercise as any} lang={chapter.lang} onResult={handleResult} onNext={handleNext} onBack={handleBack} />
          )}
          {exerciseType === 'matching' && (
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            <MatchingExercise data={exercise as any} lang={chapter.lang} onResult={handleResult} onNext={handleNext} onBack={handleBack} />
          )}
          {exerciseType === 'trueFalse' && (
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            <TrueFalseExercise data={exercise as any} lang={chapter.lang} onResult={handleResult} onNext={handleNext} onBack={handleBack} />
          )}
          {exerciseType === 'fillBlank' && (
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            <FillBlankExercise data={exercise as any} lang={chapter.lang} onResult={handleResult} onNext={handleNext} onBack={handleBack} />
          )}
