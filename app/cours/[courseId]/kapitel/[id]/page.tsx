...
export default function ChapterPage() {
  ...
  useEffect(() => {
    Promise.resolve().then(() => {
      if (!isSupabaseConfigured()) {
        setProgress(localGetProgress().filter(r => r.course_id === courseId && r.chapter_id === chapterId))
        return
      }
      getSupabaseClient().auth.getUser().then(async ({ data }) => {
        if (!data.user) {
          setProgress(localGetProgress().filter((r: ProgressRow) => r.course_id === courseId && r.chapter_id === chapterId))
          return
        }
        try {
          const rows = await getProgress(data.user.id)
          setProgress((rows as ProgressRow[]).filter(r => r.course_id === courseId && r.chapter_id === chapterId))
        } catch (error) {
  ...
  if (!course || !chapter) return <div className="text-center py-10">Chapitre non trouvé</div>
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-7 sm:space-y-8">
      ...
      <div className="flex items-start gap-3 sm:gap-4">
        <Link href={`/cours/${courseId}`} className="mt-1 inline-flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-md transition-colors hover:bg-accent">
          <ArrowLeft className="h-4 w-4" />
        </Link>
        <div className="min-w-0 flex-1">
          <div className="mb-2 flex flex-wrap items-center gap-2">
            <Badge variant="secondary">{course.de} · Kapitel {chapter.id}</Badge>
          </div>
